'use client'
//https://github.com/privy-io/base-paymaster-example
import React, { useState, useEffect, useContext, useMemo } from 'react'
import { ConnectedWallet, useWallets } from '@privy-io/react-auth'
import { Client, RpcTransactionRequest, createPublicClient, createWalletClient, custom, http, WalletClient } from 'viem'
...
export const SmartAccountProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [eoaClient, setEoaClient] = useState<WalletClient | undefined>()
  ...
  const bundler: PublicErc4337Client = useMemo(
    () =>
      createPublicErc4337Client({
        chain: baseGoerli,
        rpcUrl: `${BASE_GOERLI_ALCHEMY_RPC_URL}/${process.env.NEXT_PUBLIC_ALCHEMY_API_KEY as string}`,
      }),
    []
  )
  ...
  useEffect(() => {
    ...
    const createSmartWallet = async (eoa: ConnectedWallet) => {
      ...
      const eoaClient: WalletClient = createWalletClient({
        account: eoa.address as `0x${string}`,
        chain: baseGoerli,
        transport: custom(eoaProvider),
      })

      setEoaClient(eoaClient)

      await PushAPI.initialize(eoaClient as viemSignerType, { env: CONSTANTS.ENV.STAGING })
